...
private enum RequiredArg(Config config, ArgumentInfo info, size_t index) =
    (in bool[size_t] cliArgs)
    {
        return (index in cliArgs) ?
            Result.Success :
            Result.Error("The following argument is required: '", config.helpStyle.namedArgumentName(info.displayName), "'");
    };
    ...
    (in bool[size_t] cliArgs, in size_t[] restrictionArgs)
    {
        ...
        foreach(index; restrictionArgs)
        {
            ...
            if(foundIndex != size_t.max && missedIndex != size_t.max)
                return Result.Error("Missed argument '", config.helpStyle.namedArgumentName(allArgs[missedIndex].displayName),
                    "' - it is required by argument '", config.helpStyle.namedArgumentName(allArgs[foundIndex].displayName), "'");
        }
...
private enum RequiredAnyOf(Config config, ArgumentInfo[] allArgs) =
    (in bool[size_t] cliArgs, in size_t[] restrictionArgs)
    {
        ...
        return Result.Error("One of the following arguments is required: '",
            restrictionArgs.map!(_ => config.helpStyle.namedArgumentName(allArgs[_].displayName)).join("', '"), "'");
    };
        ...
        foreach(index; restrictionArgs)
            if(index in cliArgs)
            {
                if(foundIndex == size_t.max)
                    ...
                    return Result.Error("Argument '", config.helpStyle.namedArgumentName(allArgs[foundIndex].displayName),
                        "' is not allowed with argument '", config.helpStyle.namedArgumentName(allArgs[index].displayName),"'");
            }
