...
package struct Arguments
{
    ...
    void add(Config config, TYPE, ArgumentInfo info)()
    {
        static if(hasMemberGroupUDA!(TYPE, info.memberSymbol))
        {
            ...
            if(index !is null)
                addArgumentImpl!(TYPE, info)(userGroups[*index]);
            else
            {
                groupsByName[group.name] = userGroups.length;
                userGroups ~= group;
                addArgumentImpl!(TYPE, info)(userGroups[$-1]);
            }
        }
        else static if(info.required)
            addArgumentImpl!(TYPE, info)(requiredGroup);
        else
            addArgumentImpl!(TYPE, info)(optionalGroup);
    }

    private void addArgumentImpl(TYPE, ArgumentInfo info)(ref Group group)
    {
