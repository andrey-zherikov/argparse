...
package(argparse) struct ArgumentInfo
{
    ...
    auto checkValuesCount(const Config* config, string paramName, size_t numValues) const
    {
        ...
        if(min == max && numValues != min)
            return Result.Error("Argument '",config.styling.argumentName(paramName),"': expected ",min,min == 1 ? " value" : " values");

        if(numValues < min)
            return Result.Error("Argument '",config.styling.argumentName(paramName),"': expected at least ",min,min == 1 ? " value" : " values");

        if(numValues > max)
            return Result.Error("Argument '",config.styling.argumentName(paramName),"': expected at most ",max,max == 1 ? " value" : " values");
